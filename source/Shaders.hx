...
class BarrelDistortionEffect extends Effect
{
	...
	public var shader:BarrelDistortionShader;
	public function new (value1:Float, value2:Float){
		shader = new BarrelDistortionShader();
		shader.dis1.value = [value1];
		shader.dis2.value = [value2];
	}
}*/

class ParticlesShader extends FlxShader
{
	@:glFragmentSource('
	#pragma header
...
class ParticlesEffect extends Effect{
	public var shader:ParticlesShader;
	public function new (){
		shader = new ParticlesShader();
		shader.time.value = [0];
}
}


class Glitcheadoshader extends FlxShader
{
	@:glFragmentSource('
...
class GlitcheadoEffect extends Effect{
...
//0.05, 250.0. 0.0088, 0.02, 1.3
	
	public var shader:Glitcheadoshader;
	public function new (daMenu:String = "", blurAmount:Float, noiseq:Float, noiseI:Float, offsetI:Float, colorOI:Float){
		shader = new Glitcheadoshader();
		shader.iTime.value = [0];
		shader.range.value = [blurAmount];
		shader.noiseQuality.value = [noiseq];
		shader.noiseIntensity.value = [noiseI];
		shader.offsetIntensity.value = [offsetI];
		shader.colorOffsetIntensity.value = [colorOI];
	switch (daMenu.toLowerCase())
	{
		default:
...
class CRDShader extends FlxShader
{
...
class CRDEffect extends Effect
{
	public var shader:CRDShader;
	public function new(daMenu:String = ""){
		shader = new CRDShader();
		shader.iTime.value = [0];
...
class VCRDistortionEffect extends Effect
{
  ...
  public function new(glitchFactor:Float,distortion:Bool=true,perspectiveOn:Bool=true,vignetteMoving:Bool=true, daMenu:String = ""){
    shader.iTime.value = [0];
   ...
   switch (daMenu.toLowerCase())
	{
		//case 'menu':
			//MainMenuState.shaderUpdate.push(update);
		/*case 'nome que voce quiser': exemplo!11!
			NomeDoSeuState.shaderUpdate.push(update);*/
		default:
			PlayState.instance.shaderUpdates.push(update);
	}
  }
...
class MissignoEffect extends Effect{
	public var shader:MissignoShader = new MissignoShader();
	public function new (binaryAmount:Float){
		shader.binaryIntensity.value = [binaryAmount];
	}
}

class MissignoShader extends FlxShader{
